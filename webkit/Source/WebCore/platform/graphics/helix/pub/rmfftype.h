/* ***** BEGIN LICENSE BLOCK *****
 * Source last modified: $Id: rmfftype.h,v 1.14.2.1.40.1 2010/12/23 02:00:50 ching_li Exp $
 * 
...
{
    // Since invoker might ignore the return value, to be on the
    // safe side, reset all data to default values.
    theID                 = 0;
    AudioSpecificVersion  = 0;
    AudioSpecificRevision = 0;
    raID                  = 0;
    RaChunkSize           = 0;
    version               = 0;
    revision              = 0;
    ...
    bytesPerMinute2       = 0;
    interleaveFactor      = 0;
    interleaveBlockSize   = 0;
    userData              = 0;
    sampleRate            = 0;
    sampleSize            = 0;
    numChannels           = 0;
    memset(InterleaveCode,  0, 5);
    memset(CompressionCode, 0, 5);
    isInterleaved         = 0;
    copyByte              = 0;
    streamType            = 0;
    title_len             = 0;
    ...
    app_len               = 0;
    app                   = NULL;
    if (!buf || len < sizeof(UINT32)*9 + sizeof(UINT16)*10 + 13)
    {
        return NULL;
    }
    UINT8* off = buf;
