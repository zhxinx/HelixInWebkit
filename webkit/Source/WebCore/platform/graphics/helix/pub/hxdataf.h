/* ***** BEGIN LICENSE BLOCK *****
 * Source last modified: $Id: hxdataf.h,v 1.9.84.3 2012/05/03 03:09:38 shuohuang Exp $
 * 
...
#if defined(_WIN64)  || defined(_WINDOWS)
#define HXSTAT struct _stat64
#elif defined(ANDROID)
// Android does not have stat64, or it does, but it just defines it back to 'stat'.
// Look at the source: bionic/libc/include/sys/stat.h
// We get some weird compile errors in places if it is defined to stat64 since it 
// redefines it back to stat, which uses long long for the size
...
typedef _INTERFACE	IHXDataFileFactory2		IHXDataFileFactory2;
...
/* {5BBA1496-D2EB-11DE-BFCB-FCCF55D89593} */
DEFINE_GUID(IID_IHXDataFileFactory2,	0x5BBA1496, 0xD2EB, 0x11DE, 0xBF, 0xCB, 
					0xFC, 0xCF, 0x55, 0xD8, 0x95, 0x93);
#undef  INTERFACE
#define INTERFACE   IHXDataFileFactory2

DECLARE_INTERFACE_(IHXDataFileFactory2, IUnknown)
{
    /*
     *  IUnknown methods
     */
    ...
    /*
     *  IHXDataFileFactory2 methods
     */

    /* Creates a datafile for buffered or unbuffered I/O
     * By default create an IHXDataFile for BUFFERED file I/O 
     * for all platforms, except SOLARIS, where UNBUFFERED 
     * file I/O is required. Extended version with bUseFd.
     */
    STDMETHOD(CreateFileEx)		(THIS_
					REF(IHXDataFile*) pDataFile,
					IUnknown*  pContext,
					REF(IUnknown*) pPersistantObject,
					HXBOOL bDisableMemoryMappedIO,
