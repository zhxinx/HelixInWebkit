/* ***** BEGIN LICENSE BLOCK *****
 * Source last modified: $Id: hxausvc.h,v 1.19.2.2.2.5 2011/11/14 07:28:05 xzhao Exp $
 * 
...
typedef _INTERFACE   IHXAudioEqualizer		    IHXAudioEqualizer;
typedef _INTERFACE   IHXAudioFormatSwitchRuntime    IHXAudioFormatSwitchRuntime;
...
typedef _INTERFACE   IHXAudioDeviceDirectMode       IHXAudioDeviceDirectMode;
typedef _INTERFACE   IHXAudioDeviceResponse	    IHXAudioDeviceResponse;

// Device volume changed, device mute status changed, device error occurred, etc.
typedef _INTERFACE   IHXAudioDeviceResponse2        IHXAudioDeviceResponse2;
...
typedef struct _HXAudioFormat
{
    ...
    UINT32	uMaxBlockSize;	/* Max Blocksize			    */
} HXAudioFormat;
 ...
 *  Interface:
 * 
 *  IHXAudioFormatSwitchRuntime
 * 
 *  Purpose:
 * 
 *  This interface provides ability to let audio system (player/stream/session/driver) to 
 * switch to a new audio format in the run-time
 * 
 *  IID_IHXAudioFormatSwitchRuntime:
 * 
 *  368cd6b8-c4ce-4792-a339-06a94663a997
 * 
 */
DEFINE_GUID(IID_IHXAudioFormatSwitchRuntime, 0x368cd6b8, 0xc4ce, 0x4792,
            0xa3, 0x39, 0x06, 0xa9, 0x46, 0x63, 0xa9, 0x97);

#undef  INTERFACE
#define INTERFACE   IHXAudioFormatSwitchRuntime

DECLARE_INTERFACE_(IHXAudioFormatSwitchRuntime, IUnknown)
{
    /*
     *  IUnknown methods
     */
    ...
    /*
     *  IHXAudioFormatSwitchRuntime method
     */
    STDMETHOD(SwitchToNewAudioFormat) (THIS_ HXAudioFormat* format) PURE;
};
 ...
 *  Purpose:
 * 
 *  This interface provides access to the Audio equalizer configuration . 
 *  This service allows users to turn on/off, get/set properties such 
 *  numOfBands/Gain.
 * 
 *  IHXAudioEqualizer:
 * 
 *  E5E089EA-C10E-11DF-A72D-6CF049264A26
 * 
 */
DEFINE_GUID(IID_IHXAudioEqualizer, 0xE5E089EA, 0xC10E, 0x11DF, 0xA7, 0x2D, 0x6C, 0xF0, 0x49, 0x26, 0x4A, 0x26);

#undef  INTERFACE
#define INTERFACE   IHXAudioEqualizer

DECLARE_INTERFACE_(IHXAudioEqualizer, IUnknown)
{
    /*
     *  IUnknown methods
     */
    ...
    /*
     *  IHXAudioEqualizer method
     */
    /************************************************************************
    *  Method:
    *      IHXAudioEqualizer::OnOffEqualizer
    *  Purpose:
    *	   Allow user to turn on/off the equalizer
    */
    STDMETHOD(OnOffEqualizer)    (THIS_
                                  HXBOOL   bEnable
                                  ) PURE;

    /************************************************************************
    *  Method:
    *      IHXAudioEqualizer::SetProperty
    *  Purpose:
    *	   Allow user to set the property of a specific equalizer band
    */
    ...
    /************************************************************************
    *  Method:
    *      IHXAudioEqualizer::GetProperty
    *  Purpose:
    *	   Allow user to get the property of a specific equalizer band
    */
    ...
    /************************************************************************
    *  Method:
    *      IHXAudioEqualizer::SetNumOfBands
    *  Purpose:
    *	   Allow user to set the number of bands of the equalizer
    */
    STDMETHOD(SetNumOfBands)    (THIS_
                               UINT8       uBandsNum
                              ) PURE;

    /************************************************************************
    *  Method:
    *      IHXAudioEqualizer::GetNumOfBands
    *  Purpose:
    *	   Allow user to get the number of bands of the equalizer
    */
    STDMETHOD(GetNumOfBands)    (THIS_
                               UINT8      *pBandsNum
                              ) PURE;
};
 ...
 *  Interface:
 * 
 *  IHXAudioDeviceResponse2
 * 
 *  Implemented by: Helix Audio Session
 *  Used by:        Helix Audio Device
 *
 *  Purpose:
 *
 *  This interface is used by the IHXAudioDevice to report (system) actions 
 *  from outside of Helix that cause changes otherwise not noticed by Helix. 
 *
 *  The asynchronous changes in the audio device reported with this interface are: 
 *  (1) volume changes controlled from system volume control, external to Helix.
 *  (2) losing access to audio device (playback terminated from the device's system dialog box)
 * 
 *  IID_IHXAudioDeviceResponse2:
 * 
 *  {361B411A-C878-4E9C-8483-DCC4AC4A7838}
 * 
 */
DEFINE_GUID(IID_IHXAudioDeviceResponse2, 0x361B411A, 0xC878, 0x4E9C, 0x84, 0x83, 
        0xDC, 0xC4, 0xAC, 0x4A, 0x78, 0x38);

#undef  INTERFACE
#define INTERFACE   IHXAudioDeviceResponse2

DECLARE_INTERFACE_(IHXAudioDeviceResponse2, IUnknown)
{
    /*
     *  IUnknown methods
     */
    ...
    /*
     *  IHXAudioDeviceResponse2 methods
     */

    /************************************************************************
     *  Method:
     *      IHXAudioDeviceResponse2::OnDeviceVolumeChange
     *  Purpose:
     *      
     *      Audio device volume changed.
     *      (Eg. User set the device volume using system volume control, outside of Helix.)
     *      When Helix calls IHXVolume::SetVolume(), no feedback is reflected in this method.
     */
    STDMETHOD(OnDeviceVolumeChange) (THIS_
                                    const UINT16    /*IN*/ ulVolume) PURE;

    /************************************************************************
     *  Method:
     *      IHXAudioDeviceResponse2::OnDeviceMuteChange
     *  Purpose:
     *      
     *      Audio device mute status changed.
     *      (Eg. User set the device mute status using system volume control, outside of Helix.)
     *      When Helix calls IHXVolume::SetMute(), no feedback is made from the Audio Device.
     *      
     */
    STDMETHOD(OnDeviceMuteChange)  (THIS_
                                    const HXBOOL    bMute) PURE;


    /************************************************************************
     *  Method:
     *      IHXAudioDeviceResponse2::OnDeviceErrorOccurred
     *  Purpose:
     *
     *      Playback access to audio device terminated. AccessTerminated.
     *      (Eg. User terminated Helix audio playback stream from system interfaces.)
     *
     */
    STDMETHOD(OnDeviceErrorOccurred)   (THIS_
                                       const HX_RESULT theErr) PURE;
};
 ...
 * 
 *  Interface:
 *	IHXAudioDeviceDirectMode
 * 
 *  Purpose:
 * 
 *      Set Audio Device's direct mode
 * 
 *  IID_IHXAudioDeviceDirectMode
 * 
 *  {a1b1c116-350b-438c-8fed-7861f2f25de9}
 * 
 */
DEFINE_GUID(IID_IHXAudioDeviceDirectMode, 0xa1b1c116, 0x350b, 0x438c,
            0x8f, 0xed, 0x78, 0x61, 0xf2, 0xf2, 0x5d, 0xe9);

#undef  INTERFACE
#define INTERFACE   IHXAudioDeviceDirectMode

DECLARE_INTERFACE_(IHXAudioDeviceDirectMode, IUnknown)
{
    /*
     *  IUnknown methods
     */
    ...
    /*
     *  IHXAudioDeviceDirectMode methods
     */

    /************************************************************************
     *  Method:
     *      IHXAudioDeviceDirectMode::SetDirectMode
     *  Purpose:
     *      Set audio device's direct mode
     *      Note: In order this mode to take effect, need to set this before audio driver init
     */
    STDMETHOD(SetDirectMode) (THIS_ HXBOOL /*IN*/ bDirectMode ) PURE;


	/************************************************************************
     *  Method:
     *      IHXAudioDeviceDirectMode::SetDirectModeBasedOnEngineCoreType
     *  Purpose:
     *      Set audio device working DirectMode is under fullcore or simplecore engine;
     *      Note: the default is under fullcore engine;
     */

	STDMETHOD(SetDirectModeBasedOnEngineCoreType) (THIS_ UINT8/*IN*/ nDirectModeBasedOnEngineCoreType ) PURE;
...
DEFINE_SMART_PTR(IHXAudioFormatSwitchRuntime)
DEFINE_SMART_PTR(IHXAudioEqualizer)
DEFINE_SMART_PTR(IHXAudioPlayerResponse)
DEFINE_SMART_PTR(IHXAudioStream)
DEFINE_SMART_PTR(IHXAudioDevice)
DEFINE_SMART_PTR(IHXAudioDeviceDirectMode)
