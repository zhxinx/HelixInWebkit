/* ***** BEGIN LICENSE BLOCK *****
 * Source last modified: $Id: hxfiles.h,v 1.16.2.1.16.4 2012/05/03 03:09:38 shuohuang Exp $
 * 
...
typedef _INTERFACE      IHXFileObjectSync               IHXFileObjectSync;
typedef _INTERFACE      IHXFileObject2                  IHXFileObject2;
typedef _INTERFACE      IHXFileObjectExt                IHXFileObjectExt;
typedef _INTERFACE      IHXFileObjectExt2               IHXFileObjectExt2;
...
typedef _INTERFACE      IHXFileStat2                    IHXFileStat2;
typedef _INTERFACE      IHXFileStatResponse             IHXFileStatResponse;
typedef _INTERFACE      IHXFileStat2Response            IHXFileStat2Response;
...
#define HX_FILEADVISE_NETWORKACCESS		6
#define HX_FILEADVISE_FASTSTART			7
...
#define HX_FILERESPONSEADVISE_NETWORKACCESS	2
#define HX_FILERESPONSEADVISE_FASTSTART		3
#define HX_FILERESPONSEADVISE_LOCALACCESSONLY	4
...
#elif defined(HELIX_FEATURE_64_BIT_FILE_SUPPORT)
#define HX_STAT_TIME_T UINT64
#define HX_STAT_MODE_T UINT64
#else
...
#ifndef HXSTAT
#if defined(_WIN64) || defined(HELIX_FEATURE_64_BIT_FILE_SUPPORT)
#if defined(_WINDOWS)
#define HXSTAT struct _stat64
#else
#define HXSTAT struct stat64
#endif
#else
#define HXSTAT struct stat
#endif
#endif
 ...
 *      Object that exports file control API
 *
 *  IID_IHXFileObjectSync:
 *
 *      {9e1c07c1-ea39-40c8-95a6-b5be7527a148}
 *
 */
DEFINE_GUID(IID_IHXFileObjectSync, 0x9e1c07c1, 0xea39, 0x40c8, 0x95, 0xa6, 0xb5,
                        0xbe, 0x75, 0x27, 0xa1, 0x48);

#undef  INTERFACE
#define INTERFACE   IHXFileObjectSync

DECLARE_INTERFACE_(IHXFileObjectSync, IHXFileObject)
{
    /*
     *  IUnknown methods
     */
    ...
    STDMETHOD(InitSync) (THIS_
                        ULONG32             /*IN*/  ulFlags) PURE;
                        
		// Synchronous close                         
    STDMETHOD(CloseSync)    (THIS) PURE;                        
    
    // Reads Sync 32 bit data.
    STDMETHOD(Read)     (THIS_ REF(IHXBuffer*) pBuffer, UINT32 ulCount) PURE;
    
    // Sync 32 bit seek
    // for synchronous seek, bSync = TRUE 
    /*
    bSync = FALSE is equivalent to IHXFileObject::Seek 
    */
    STDMETHOD(Seek)     (THIS_
                        UINT32 ulOffset,
                        HXBOOL    bRelative,
	...
	STDMETHOD_(UINT32,GetFilePosition32)          (THIS) PURE;
    
    // Returns the size of the file
    STDMETHOD_(UINT32,GetFileSize32)	          (THIS) PURE;
};
 ...
 *  IID_IHXFileObject2:
 *
 *      {23192673-154c-4fb4-9ae9-b7caed05bd55}
 *
 */
DEFINE_GUID(IID_IHXFileObject2, 0x23192673, 0x154c, 0x4fb4, 0x9a, 0xe9, 0xb7,
                        0xca, 0xed, 0x05, 0xbd, 0x55);

#undef  INTERFACE
#define INTERFACE   IHXFileObject2

DECLARE_INTERFACE_(IHXFileObject2, IHXFileObjectSync)
{
    /*
     *  IUnknown methods
     */
    ...
    // Reads ASync 64 bit data.
    STDMETHOD(Read)     (THIS_
                        UINT64 ulCount) PURE;
                        
    // Reads Sync 64 bit data.
    STDMETHOD(Read)     (THIS_ REF(IHXBuffer*) pBuffer, UINT64 ulCount) PURE;
    
    // Sync\ASync 64 bit, Sync 32 bit write
    // for synchronous write, bSync = TRUE 
    // for synchronous write, bSync = FALSE 
    /*
    bSync = FALSE is equivalent to IHXFileObject::Write for 32 bit write 
    */
    STDMETHOD(Write)    (THIS_ REF(IHXBuffer*) pBuffer, HXBOOL    bSync) PURE; 
                        
    // Sync\ASync 64 bit seek
    // for synchronous seek, bSync = 1 
    // for synchronous seek, bSync = 0 
    STDMETHOD(Seek)     (THIS_
                        UINT64 ulOffset,
                        HXBOOL    bRelative,
                        HXBOOL    bSync) PURE;
		...
		// Returns the current position in the file.
	STDMETHOD_(UINT64,GetFilePosition)          (THIS) PURE;
    
    // Returns the size of the file
    STDMETHOD_(UINT64,GetFileSize64)	          (THIS) PURE;
};
#endif
 ...
 *  Object that exports file control API
 *
 *      IID_IHXFileObjectExt2:          {13416e8b-8edd-4ff9-8daf-c5bfbe3238a5}
 *
 *
 */
 DEFINE_GUID(IID_IHXFileObjectExt2, 0x13416e8b, 0x8edd, 0x4ff9, 0x8d, 0xaf, 0xc5, 0xbf, 0xbe, 0x32, 0x38, 0xa5);

#undef  INTERFACE
#define INTERFACE   IHXFileObjectExt2

DECLARE_INTERFACE_(IHXFileObjectExt2, IUnknown)
{
    /*
     *  IUnknown methods
     */
    ...
    /************************************************************************
     * Method:
     *     IHXFileObjectExt2::openurl
     * Purpose:
     *     open a new url for read
     * Paremeter:
     *    bReconnect: TRUE,  force to reconnect the server(not reuse last socket) 
     *                FALSE, reuse last socket to download data
     */
    STDMETHOD(OpenURL)   (THIS_ const char*  /*IN*/  pURL, HXBOOL /*IN*/  bReconnect) PURE;
...
DECLARE_INTERFACE_(IHXFastFileStats, IUnknown)
{
    ...
    STDMETHOD(UpdateFileObjectStats)    (THIS_
                                         HX_OFF_T         /*IN*/  nFastFileBytesSaved,
                                         HX_OFF_T         /*IN*/  nFastFileBytesNeeded) PURE;
};
...
#define CLSID_IHXRM2Converter2 IID_IHXRM2Converter2
...
#include "hxcomptr.h"
DEFINE_SMART_PTR(IHXFileObject)
DEFINE_SMART_PTR(IHXFileObjectExt)
DEFINE_SMART_PTR(IHXFileObjectExt2)
DEFINE_SMART_PTR(IHXFileResponse)
DEFINE_SMART_PTR(IHXAdvise)
DEFINE_SMART_PTR(IHXFileSystemObject)
DEFINE_SMART_PTR(IHXFileStat)
DEFINE_SMART_PTR(IHXFileStatResponse)
DEFINE_SMART_PTR(IHXFileSystemManager)
DEFINE_SMART_PTR(IHXFileSystemManagerResponse)
DEFINE_SMART_PTR(IHXFileExists)
DEFINE_SMART_PTR(IHXFileExistsResponse)
DEFINE_SMART_PTR(IHXFileMimeMapper)
DEFINE_SMART_PTR(IHXFileMimeMapperResponse)
DEFINE_SMART_PTR(IHXBroadcastMapper)
DEFINE_SMART_PTR(IHXBroadcastMapperResponse)
DEFINE_SMART_PTR(IHXGetFileFromSamePool)
DEFINE_SMART_PTR(IHXGetFileFromSamePoolResponse)
DEFINE_SMART_PTR(IHXFileAuthenticator)
DEFINE_SMART_PTR(IHXRequestHandler)
DEFINE_SMART_PTR(IHXRequestContext)
DEFINE_SMART_PTR(IHXRequest)
DEFINE_SMART_PTR(IHXFileRename)
DEFINE_SMART_PTR(IHXFileMove)
DEFINE_SMART_PTR(IHXDirHandler)
DEFINE_SMART_PTR(IHXDirHandlerResponse)
DEFINE_SMART_PTR(IHXGetRecursionLevel)
DEFINE_SMART_PTR(IHXFileRestrictor)
DEFINE_SMART_PTR(IHXFileRemove)
DEFINE_SMART_PTR(IHXFastFileFactory)
DEFINE_SMART_PTR(IHXFastFileFactory2)
DEFINE_SMART_PTR(IHXFilePlacementRead)
DEFINE_SMART_PTR(IHXFastFileStats)
DEFINE_SMART_PTR(IHXHTTPPostObject)
DEFINE_SMART_PTR(IHXHTTPPostResponse)
DEFINE_SMART_PTR(IHXHTTPRedirect)
DEFINE_SMART_PTR(IHXHTTPRedirectResponse)
DEFINE_SMART_PTR(IHXRM2Converter2)
DEFINE_SMART_PTR(IHXRM2Converter2Response)
DEFINE_SMART_PTR(IHXPoolPathAdjustment)
DEFINE_SMART_PTR(IHXPostDataHandler)
 ...
 *  Purpose:
 *
 *      Gets extended information about a specific File object
 *
 *  IID_IHXFileStat2:
 *
 *      {00002f03-0901-11d1-8B06-00A024406D59}
 *
 */
DEFINE_GUID(IID_IHXFileStat2, 0x00002F03, 0x901, 0x11d1, 0x8b, 0x6, 0x0,
                        0xa0, 0x24, 0x40, 0x6d, 0x59);

#undef  INTERFACE
#define INTERFACE   IHXFileStat2

DECLARE_INTERFACE_(IHXFileStat2, IUnknown)
{
    /*
     *  IUnknown methods
     */
 ...
 *  Interface:
 *
 *      IHXFileStat2Response
 *
 *  Purpose:
 *
 *      Returns extended information about a specific File object
 *
 *  IID_IHXFileStat2Response:
 *
 *      {00002f04-0901-11d1-8B06-00A024406D59}
 *
 */
DEFINE_GUID(IID_IHXFileStat2Response, 0x00002F04, 0x901, 0x11d1, 0x8b, 0x6, 0x0,
                        0xa0, 0x24, 0x40, 0x6d, 0x59);

#undef  INTERFACE
#define INTERFACE   IHXFileStat2Response

DECLARE_INTERFACE_(IHXFileStat2Response, IUnknown)
{
    /*
     *  IUnknown methods
     */
