/* ***** BEGIN LICENSE BLOCK *****
 * Source last modified: $Id: hxcore.h,v 1.23.2.1.2.5 2012/02/09 03:48:09 joeli Exp $
 * 
...
typedef _INTERFACE IHXMultiTrackSelection          IHXMultiTrackSelection;
typedef _INTERFACE IHXSubtitleCtrl                 IHXSubtitleCtrl;
...
DECLARE_INTERFACE_(IHXPlayer, IUnknown)
{
    ...
    STDMETHOD(Seek) (THIS_ ULONG32 ulTime, HXBOOL bAccuratePos = TRUE) PURE;
#else
 ...
 *  Interface:
 *
 *      IID_IHXMultiTrackSelection
 *
 *  Purpose:
 *
 *      Interface to allow dynamic switch stream, such as English/Chinese audio stream
 *
 *      {97881faa-b026-4357-95cf-e94c7ab2a869}
 *
 */
DEFINE_GUID(IID_IHXMultiTrackSelection, 0x97881faa, 0xb026, 0x4357, 0x95, 0xcf, 0xe9, 0x4c, 0x7a, 0xb2, 0xa8, 0x69);

#undef  INTERFACE
#define INTERFACE   IHXMultiTrackSelection

DECLARE_INTERFACE_(IHXMultiTrackSelection, IUnknown)
{
    /************************************************************************
     *	Method:Purpose
     *      iTrkType is the enum of Multi-Track type, such as MTRACK_AUDIO,MTRACK_SUBTITLE
     *	    IHXPlayer::GetTrackInfoSize
     *	        Get all audio/subtitle trackinfo size
     *      IHXPlayer::GetAllTracksInfo
     *          Fill the audio/subtitle trackinfo buffer according the above trackinfo size
     *      IHXPlayer::GetCurrentTrackID
     *          Get the Current in-use audio/subtitle track ID
     *      IHXPlayer::SetCurrentTrackID
     *          Set the Current audio/subtitle track ID and switch to the track
     */
    STDMETHOD (GetTrackInfoSize) (THIS_  UINT16 iTrkType, UINT16 *pSize) PURE;
    STDMETHOD (GetAllTracksInfo) (THIS_  UINT16 iTrktype, UINT16 *pTrkNum, char *pTrkInfo) PURE;
    STDMETHOD (GetCurrentTrackID)(THIS_  UINT16 iTrkType, UINT16 *pTrkID) PURE;
    STDMETHOD (SetCurrentTrackID)(THIS_  UINT16 iTrkType, UINT16 trkID) PURE;
};
 ...
 *  Purpose:
 *
 *      Interface to control embedded subtitle display
 *      {07f886b1-81e4-4fc4-ba1c-27f448bef2cb}
 *
 */
DEFINE_GUID(IID_IHXSubtitleCtrl, 0x07f886b1, 0x81e4, 0x4fc4, 0xba, 0x1c, 0x27, 0xf4, 0x48, 0xbe, 0xf2, 0xcb);

#undef  INTERFACE
#define INTERFACE   IHXSubtitleCtrl

DECLARE_INTERFACE_(IHXSubtitleCtrl, IUnknown)
{
    /************************************************************************
     *	Method:Purpose
     *	    IHXPlayer::IsSubtitleShowing()
     *	        Get the status of subtitle display
     *      IHXPlayer::ShowSubtitle(HXBOOL bshow)
     *          Set the status of subtitle display
     */
    STDMETHOD_(HXBOOL,IsSubtitleShowing) (THIS) PURE;
    STDMETHOD (ShowSubtitle) (THIS_ HXBOOL bShow) PURE;
    STDMETHOD (SetSubtitleColor) (THIS_ UINT32 color) PURE;
};
...
DEFINE_SMART_PTR(IHXMultiTrackSelection)
DEFINE_SMART_PTR(IHXSubtitleCtrl)
